...
export default function Inventory() {
  ...
  function getStockLevel(product: Product) {
    ...
    if (available <= 0) return { level: 'out' as const, color: 'text-red-600', bg: 'bg-red-500', barBg: 'bg-red-500/20', label: 'Out of stock', pct: 0 };
    if (available <= product.lowStockThreshold) return { level: 'low' as const, color: 'text-amber-600', bg: 'bg-amber-500', barBg: 'bg-amber-500/20', label: 'Low stock', pct: Math.min((available / Math.max(product.lowStockThreshold * 3, 1)) * 100, 100) };
    return { level: 'healthy' as const, color: product.isBundle ? 'text-violet-600' : 'text-emerald-600', bg: product.isBundle ? 'bg-violet-500' : 'bg-emerald-500', barBg: product.isBundle ? 'bg-violet-500/20' : 'bg-emerald-500/20', label: 'In stock', pct: Math.min((available / Math.max(product.lowStockThreshold * 3, 1)) * 100, 100) };
  }
